...
for room in rooms:
	...
	if searchWord.lower() in roomName.lower():
		...
		for bSegment in outerBoundaries:
			oldwall = doc.GetElement(bSegment.ElementId)
			oldWalls.append(oldwall)
		...
		rvtiListCrvLoop = List[DB.CurveLoop]()
		rvtiListCrvLoop.Add(curveLoop)
		
		newFloor = DB.Floor.Create(doc, rvtiListCrvLoop, floorType.Id, room.Level.Id)
		newFloors.append(newFloor)
doc.Regenerate()
for oldW, newW in zip(oldWalls, newWalls):
	DB.JoinGeometryUtils.JoinGeometry(doc, oldW, UnwrapElement(newW))
for oldF in floors:
	for newF in newFloors:
		try:
			DB.JoinGeometryUtils.JoinGeometry(doc, newF, oldF)
		except: pass
		
TransactionManager.Instance.TransactionTaskDone()

# Assign your output to the OUT variable.
OUT = newWalls, newFloors